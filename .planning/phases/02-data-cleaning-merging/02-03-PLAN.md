...
must_haves:
  truths:
    - "Encounters merge with diagnoses by patid + encounterid producing expected row growth"
    - "Encounters merge with procedures by patid + encounterid"
    - "Encounters merge with provider data by providerid"
    - "Row counts are logged before and after every merge operation"
    - "Many-to-many relationships are detected and prevented via dplyr relationship argument"
    - "Post-merge assertr assertions verify no unexpected NAs and correct value ranges"
  artifacts:
    - path: "R/02_merge.R"
      provides: "Complete encounter-dx-proc-provider merge pipeline with validation"
      contains: "logged_join"
      min_lines: 150
    - path: "data/processed/02_merged_enc_dx.rds"
      provides: "Encounters merged with diagnoses checkpoint"
    - path: "data/processed/02_merged_enc_dx_proc.rds"
      provides: "Encounters merged with diagnoses and procedures checkpoint"
    - path: "data/processed/02_merged_complete.rds"
      provides: "Final merged dataset with all tables joined"
  key_links:
    - from: "R/02_merge.R"
      to: "data/processed/02_encounters_cleaned.rds"
      via: "readRDS to load cleaned encounters"
      pattern: "readRDS.*02_encounters_cleaned"
    - from: "R/02_merge.R"
      to: "data/processed/02_dx_combined.rds"
      via: "readRDS to load combined diagnoses"
      pattern: "readRDS.*02_dx_combined"
    - from: "R/02_merge.R"
      to: "data/processed/02_proc_combined.rds"
      via: "readRDS to load combined procedures"
      pattern: "readRDS.*02_proc_combined"
    - from: "R/02_merge.R"
      to: "data/processed/01_imported_provider.rds"
      via: "readRDS to load provider data"
      pattern: "readRDS.*01_imported_provider"
---

<objective>
Create R/02_merge.R: Merge cleaned encounters with diagnoses, procedures, and provider data using PCORnet CDM join keys, with row count validation logging at every step, cardinality enforcement via dplyr relationship argument, and post-merge assertr data quality checks.

Purpose: This is the second production script of Phase 2 (per D-01), performing all multi-table join operations with validation. Every merge is logged, cardinality-enforced, and assertion-checked to prevent the SAS-to-R join semantic pitfalls.

Output: R/02_merge.R script that produces data/processed/02_merged_complete.rds
</objective>
...
<interfaces>
<!-- Checkpoints from 02_clean.R that this script reads -->

From R/02_clean.R (Plan 02-02 output):
```r
# data/processed/02_encounters_cleaned.rds
# Columns: patid, encounterid, enc_type, admit_date, discharge_date,
#          discharge_status, discharge_disposition, payer_type_primary,
#          payer_type_secondary, providerid, facility_type,
#          enc_type_label (factor), discharge_status_label (factor),
#          discharge_disposition_label (factor),
#          payer_primary_code, payer_primary_grouped (factor),
#          payer_secondary_code, payer_secondary_grouped (factor),
#          payer_primary_collapsed (factor), payer_primary_public (factor)

# data/processed/02_dx_combined.rds
# Combined from 7 diagnosis parts. Columns: patid, encounterid,
# diagnosisid, dx, dx_type, dx_source, dx_origin, dx_date, dx_poa,
# pdx, admit_date, enc_type, providerid, source

# data/processed/02_proc_combined.rds
# Combined from 4 procedure parts. Columns: patid, encounterid,
# proceduresid, px, px_type, px_source, px_date, enc_type, providerid
```

From R/01_import.R (Phase 1 checkpoints):
```r
# data/processed/01_imported_provider.rds
# Columns: providerid, provider_specialty_primary, ...

# data/processed/01_imported_prov_spec.rds
# Columns: provider_specialty_primary, cancer_provider,
#          provider_classification, provider_specialization
```

PCORnet CDM Join Keys (from v7.0 spec):
- ENCOUNTER <-> DIAGNOSIS: patid + encounterid
- ENCOUNTER <-> PROCEDURES: patid + encounterid
- ENCOUNTER <-> PROVIDER: providerid (encounter.providerid = provider.providerid)
- PROVIDER <-> PROV_SPEC: provider_specialty_primary

SAS merge patterns (from forensic analysis):
```sas
-- V5_3 lines 11-27: demo_validEnroll inner join diagnosis on id
-- V5_3 lines 29-50: diagnosis left join Icd10_groups2 on dx = ICD9_10
-- V5_3 lines 135-170: demo_validEnroll left join dx_1 on id
-- V5_6 lines 46-129: multiple merges building up analytical dataset by id + encounterid
```

SAS MERGE vs dplyr join CRITICAL difference:
- SAS MERGE many-to-many: produces max(m, n) rows
- dplyr join many-to-many: produces m * n rows (Cartesian product)
- FIX: Use relationship = "one-to-many" or "many-to-one" on every join
</interfaces>
</context>
...
<task type="auto">
  <name>Task 1: Create 02_merge.R — encounter-dx-proc-provider merges with validation</name>
  <files>R/02_merge.R</files>
  <read_first>
    - R/config.R (path configuration)
    - R/02_clean.R (outputs this script reads — once it exists from Plan 02-02)
    - R/01_import.R (provider checkpoint naming, save_checkpoint pattern)
    - .planning/phases/02-data-cleaning-merging/02-CONTEXT.md (decisions D-01, D-05, D-06)
    - .planning/phases/02-data-cleaning-merging/02-RESEARCH.md (Pattern 5: PCORnet join keys, Pattern 6: row count validation, Pattern 7: assertr assertions, Pitfall 1: SAS MERGE vs dplyr)
    - SAS_CODE_FOR_V5_3.sas (diagnosis merge SAS pattern, lines 11-50 and 135-170)
    - SAS_CODE_FOR_V5_6.sas (encounter-based merge pattern, lines 46-129)
  </read_first>
  <action>
Create R/02_merge.R with the following exact structure.
...
#          data/processed/02_merged_enc_dx_proc.rds
...
```
  ...
  <verify>
    <automated>Rscript -e "library(testthat); test_file('tests/testthat/test_02_merging.R')"</automated>
  </verify>
  <acceptance_criteria>
    - R/02_merge.R exists and is >= 150 lines
    - R/02_merge.R contains `source(here::here("R", "config.R"))` at the top
    - R/02_merge.R contains `readRDS(file.path(data_dir_processed, "02_encounters_cleaned.rds"))` to load cleaned data
    - R/02_merge.R contains `readRDS(file.path(data_dir_processed, "02_dx_combined.rds"))` to load diagnoses
    - R/02_merge.R contains `readRDS(file.path(data_dir_processed, "02_proc_combined.rds"))` to load procedures
    - R/02_merge.R contains `readRDS(file.path(data_dir_processed, "01_imported_provider.rds"))` for provider data
    - R/02_merge.R contains `readRDS(file.path(data_dir_processed, "01_imported_prov_spec.rds"))` for provider specialty
    - R/02_merge.R contains `logged_join` function definition with parameters x, y, by, type, relationship, desc
    - R/02_merge.R contains `relationship = "one-to-many"` in encounter-diagnosis join per MRG-03
    - R/02_merge.R contains `relationship = "many-to-one"` in provider-specialty join
    - R/02_merge.R contains `by = c("patid", "encounterid")` for encounter-diagnosis join per MRG-01
    - R/02_merge.R contains `by = "providerid"` for encounter-provider join per MRG-01
    - R/02_merge.R contains `by = "provider_specialty_primary"` for provider-specialty join
    - R/02_merge.R contains `janitor::get_dupes(encounterid)` for pre-merge duplicate detection per MRG-03
    - R/02_merge.R contains `message(` calls with row counts before and after joins per MRG-02
    - R/02_merge.R contains `assertr::verify(` or `assertr::insist(` for post-merge assertions per MRG-04
    - R/02_merge.R contains `tryCatch(` wrapping assertions for warn-and-continue per D-05
    - R/02_merge.R contains `saveRDS(` calls for 02_merged_enc_dx.rds, 02_merged_enc_proc.rds, 02_provider_full.rds, 02_merged_complete.rds
    - R/02_merge.R contains `growth > 20` or equivalent Cartesian product warning per MRG-03
    - R/02_merge.R does NOT contain hardcoded absolute paths
    - R/02_merge.R contains `# SAS source:` or `# SAS MERGE` documentation per INF-04
    - R/02_merge.R contains `library(assertr)` per D-05
  </acceptance_criteria>
  <done>02_merge.R loads cleaned checkpoints, performs 4 logged merges (enc+dx, enc+proc, provider+spec, enc_dx+provider) with row count tracking at every step, enforces join cardinality via relationship argument, runs assertr post-merge assertions in warn-and-continue mode, and saves 4 .rds checkpoints. All test_02_merging.R tests pass.</done>
</task>

</tasks>

<verification>
- R/02_merge.R parses without error: `Rscript -e "parse('R/02_merge.R')"` exits 0
- Merging tests pass: `Rscript -e "library(testthat); test_file('tests/testthat/test_02_merging.R')"` shows all GREEN
- No hardcoded paths: grep for "C:", "/blue/", "/home/" in R/02_merge.R returns empty
- logged_join function exists and logs row counts
</verification>

<success_criteria>
R/02_merge.R exists with logged_join helper, 4 merge operations using PCORnet CDM keys (patid+encounterid, providerid, provider_specialty_primary), dplyr relationship argument on every join, row count logging (MRG-02), duplicate detection (MRG-03), and assertr assertions (MRG-04). All test_02_merging.R tests pass.
</success_criteria>

<output>
After completion, create `.planning/phases/02-data-cleaning-merging/02-03-SUMMARY.md`
</output>
