...
Per D-02 (Claude's discretion on placement), diagnosis/procedure concatenation belongs in 02_clean.R because:
1. It's a data preparation step (combining parts), not a merge operation
2. The combined datasets are prerequisites for 02_merge.R joins
3. Keeps 02_merge.R focused solely on join operations
...
- This task creates the file; Task 2 appends the recoding sections
  ...
  <verify>
    <automated>Rscript -e "parse('R/02_clean.R'); cat('02_clean.R parses OK\n')"</automated>
  </verify>
  <acceptance_criteria>
  ...
  <done>02_clean.R has setup, encounter import via import_sas(), encounter combination via bind_rows (CLN-02), and dx/proc concatenation. File parses without error. Ready for Task 2 recoding operations.</done>
</task>
