...
<task type="auto">
  <name>Task 1: Create 02_clean.R — encounter import, combination, and variable recoding</name>
  ...
  <read_first>
    ...
    - R/01_formats.R (all format definitions: p_payer lines 696-700, payerr lines 584-588, payerrr lines 592-596, enc_type lines 138-145, discharge_status lines 120-128, discharge_disposition lines 114-117)
    - .planning/phases/02-data-cleaning-merging/02-CONTEXT.md (decisions D-01 through D-08)
    - .planning/phases/02-data-cleaning-merging/02-RESEARCH.md (Pattern 1: encounter import, Pattern 2: payer recoding, Pattern 4: missing value handling)
  ...
  <action>
Create R/02_clean.R with the following exact structure. The script must be fully self-contained (sources config.R, loads libraries, loads formats, and produces output checkpoint).
...
source(here::here("R", "01_import.R"))  # Loads import_sas() function, haven, tidyverse, janitor
library(assertr)
```

Wait — import_sas() is defined inside 01_import.R which also EXECUTES all the imports. We must NOT re-execute all imports. Instead:
- Source config.R
- Load libraries directly: `library(tidyverse)`, `library(haven)`, `library(janitor)`, `library(assertr)`
- Copy or reference the import_sas function. **Best approach**: Extract import_sas from 01_import.R or redefine it inline.

**CORRECTION**: The cleanest approach per D-07 is to call import_sas() which is defined in 01_import.R. Since 01_import.R executes on source, we should:
1. Source config.R for paths
2. Load libraries directly
3. Load sas_formats from checkpoint
4. Define a local `import_sas_file()` function that replicates the import_sas logic (or copy the function). This avoids re-executing all Phase 1 imports.

Actually, the simplest approach: **Redefine import_sas inline in 02_clean.R** (copy the function body from 01_import.R lines 57-101). Add comment: "# Replicated from 01_import.R to avoid re-executing all Phase 1 imports".
...
- Preserve `sas_labels` attribute through pipeline: `attr(encounters, "sas_labels")` should survive dplyr operations (it may not — document this known limitation)
  ...
  <acceptance_criteria>
    - R/02_clean.R exists and is >= 150 lines
  ...
  <done>02_clean.R imports encounter1/encounter2, combines them, recodes all payer types via factor() with sas_formats$p_payer, recodes enc_type/discharge fields, handles missing values explicitly, concatenates dx/proc parts, saves 3 .rds checkpoints. All cleaning tests from test_02_cleaning.R pass.</done>
</task>
